...
func mlxStft(
  ...
  // BUILD 44 AUDIT (retained for the record): the framing arithmetic is
  // provably in-bounds — floor((len-nFft)/hop) guarantees
  // (numFrames-1)*hop + nFft <= len, and with center padding len >= nFft
  // always. BUILD 45: the asStrided-bypass lever ("Z") was FALSIFIED on
  // device (arm B: corruption at control rates with the bypass active,
  // bit-equal output proven on Mac) and is removed. asStrided is exonerated.
